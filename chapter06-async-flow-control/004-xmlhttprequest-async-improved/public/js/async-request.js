(function() {
  ...
  function httpRequest(url, callback) {
    ...
    req.addEventListener("load", function() {
      if (req.status < 400) {
        ...
        callback(null, new Error("Request failed: " + req.statusText + "(" + req.status + ")"));
      }
    });
    try {
      req.send(null);
      ...
      callback(null, new Error("Request failed: Could not send request (-1)"));
    }
  }
  ...
  button.addEventListener("click", function() {
    ...
    httpRequest("http://spark:8080/data.txt", showResults);
  });
