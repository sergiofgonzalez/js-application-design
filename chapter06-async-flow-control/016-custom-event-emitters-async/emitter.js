...
function emitter(thing) {
  ...
  thing.emit = function(type) {
    ...
    for (var i = 0; i < eventListeners.length; i++) {
      debounce(eventListeners[i]);
    }

    function debounce(eventListener) {
      setTimeout(function invokeEventListener() {
        eventListener.apply(thing, args);
      }, 0); // Call right away, but asynchronously
    }
  };
